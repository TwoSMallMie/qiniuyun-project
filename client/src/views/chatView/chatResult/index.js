 ...
 */
class chatResult {
  constructor(role, content, audio, copy, thinking, reThinking) {
    this.role = role;
...
export function createChatResult(
    role, content, 
    { audio = 'none', copy = false, thinking = false, reThinking = false } =
    { audio: 'none', copy: false, thinking: false, reThinking: false }) 
{
  return new chatResult(role, content, audio, copy, thinking, reThinking);
}
